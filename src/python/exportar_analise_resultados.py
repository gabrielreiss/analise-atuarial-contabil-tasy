...
def calcular_sinsitralidade(grupo):
    grupo = grupo.fillna(0)
    grupo['MENSALIDADE'].rolling(12, min_periods=1).sum()
    grupo['CONTA_12M'] = grupo['CONTA'].rolling(12, min_periods=1).sum()
    grupo['SUS_12M'] = grupo['SUS'].rolling(12, min_periods=1).sum()
    grupo['REEMBOLSO_12M'] = grupo['REEMBOLSO'].rolling(12, min_periods=1).sum()
    grupo['GLOSA_12M'] = grupo['GLOSA'].rolling(12, min_periods=1).sum()
    grupo['COPARTICIPACAO_12M'] = grupo['COPARTICIPACAO'].rolling(12, min_periods=1).sum()
    grupo['MENSALIDADE_12M'] = grupo['MENSALIDADE'].rolling(12, min_periods=1).sum()
    grupo['Sinistralidade_12m'] = (grupo['CONTA_12M'] + grupo['SUS_12M'] + grupo['REEMBOLSO_12M'] + grupo['GLOSA_12M'] - grupo['COPARTICIPACAO_12M']) / grupo['MENSALIDADE_12M']
    return grupo
...
def exportar_csv(nome:str):
    ...
    df = df.groupby(['PRODUTO', 'REGULAMENTACAO', 'TIPO_CONTRATACAO'], as_index=False).apply(calcular_sinsitralidade)
...
#### Salvando arquivo
exportar_csv('analise_resultado')
